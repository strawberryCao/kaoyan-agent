...
def render_fortune_card(settings: Settings) -> None:
    ...
    with tab_daily:
        if st.button(
            "抽每日签", type="primary", use_container_width=True, key="fortune_daily"
        ):
            with st.spinner("正在生成每日签..."):
                st.session_state.latest_fortune_daily = workflow.generate_daily_sign()
        render_fortune_result(
            st.session_state.get("latest_fortune_daily"), "daily_sign", workflow
        )
    ...
    with tab_random:
        ...
        render_fortune_result(
            st.session_state.get("latest_fortune_random"), "random_task", workflow
        )
    ...
    with tab_soothing:
        ...
        if st.button(
            ...
            type="primary",
            use_container_width=True,
            key="fortune_soothing",
        ):
            with st.spinner("正在生成安抚签..."):
                st.session_state.latest_fortune_soothing = (
                    workflow.generate_soothing_task(user_state or "低能量")
                )
        render_fortune_result(
            st.session_state.get("latest_fortune_soothing"), "soothing_task", workflow
        )
...
def render_fortune_result(
    item: dict | None, kind: str, workflow: PlanningWorkflow
) -> None:
    if not item:
        st.info("点击上方按钮生成结果。")
        return
...
def render_latest_fortune_item(item: dict, kind: str) -> None:
    if kind == "daily_sign":
        level_label = html.escape(sign_level_label(item.get("sign_level", "")))
        sign_text = html.escape(item.get("sign_text", ""))
        today_advice = html.escape(item.get("today_advice", ""))
        action = html.escape(item.get("action", ""))
        st.html(f"""
        <div class="kaoyan-card">
            <div class="kaoyan-card-title">每日签：{level_label}</div>
            <div><strong>签文：</strong> {sign_text}</div>
            <div><strong>今日建议：</strong> {today_advice}</div>
            <div><strong>建议行动：</strong> {action}</div>
        </div>
        """)
        return
...
def render_history_item(record: dict) -> None:
    ...
    card_type = html.escape(vm["card_type_label"])
    main_text = html.escape(vm["main_text"])
    action_text = html.escape(vm["action"]) if vm["action"] else ""
    minutes = vm["minutes"]

    html_content = f"""
    <div class="kaoyan-card">
        <div class="kaoyan-card-title">{card_type}</div>
        <div><strong>内容：</strong> {main_text}</div>
    """
    if vm["action"]:
        html_content += f"    <div><strong>建议行动：</strong> {action_text}</div>\n"
    if minutes > 0:
