...
class WorkspaceWorkflow:
    ...
    def list_tasks(
        ...
        """获取任务列表，包含 review_priority 字段"""
        tasks = self.task_repository.list(date_str=today, limit=limit)
        # 确保每个任务都有 review_priority
        for task in tasks:
            if 'review_priority' not in task:
                task['review_priority'] = task.get('review_priority', 2)
        return tasks
