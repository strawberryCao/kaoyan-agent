...
CREATE TABLE IF NOT EXISTS review_attempts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    project_id INTEGER,
    source_type TEXT NOT NULL DEFAULT 'manual',  -- manual / problem_board / review_hint
    source_id INTEGER,                           -- problem_id 或 hint_id
    subject TEXT NOT NULL DEFAULT '',
    topic TEXT NOT NULL DEFAULT '',
    question TEXT NOT NULL DEFAULT '',           -- 用户原始问题描述
    hint_content TEXT NOT NULL DEFAULT '',       -- 生成的复刷提示内容
    user_answer TEXT NOT NULL DEFAULT '',        -- 用户输入的答案/理解
    ai_feedback TEXT NOT NULL DEFAULT '',        -- AI批改反馈
    is_correct INTEGER NOT NULL DEFAULT 0,       -- 0: 错误, 1: 正确, -1: 部分正确
    confidence INTEGER NOT NULL DEFAULT 0,       -- 用户自评掌握度 0-100
    created_at TEXT NOT NULL,
    FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE SET NULL
);

CREATE INDEX IF NOT EXISTS idx_review_attempts_created_at ON review_attempts (created_at);
CREATE INDEX IF NOT EXISTS idx_review_attempts_subject ON review_attempts (subject);
CREATE INDEX IF NOT EXISTS idx_review_attempts_source ON review_attempts (source_type, source_id);

-- 给 mistake_cards 添加掌握度分数
