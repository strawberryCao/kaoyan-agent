...
from kaoyan_agent.services.memory_backend_audit import MemoryBackendAudit, NO_GRAPH_MESSAGE, NO_VECTOR_MESSAGE
...
def render_memory_system_page() -> None:
    ...
    render_page_header(
        "记忆系统",
        "审计 raw evidence、长期记忆、问题看板、向量检索和图谱数据库落地状态。",
        badge="Agent 诊断",
    )
...
def render_overview(audit: dict) -> None:
    ...
    cols = st.columns(4)
    cols[0].metric("Raw Events", counts.get("raw_events", 0))
    ...
    render_metric_card("SQLite 主库", sql.get("backend", "sqlite"), helper=f"tables: {sql.get('database_tables', 0)}")
    render_metric_card(
        "Chroma 向量库",
        "available True" if vector.get("available") else "available False",
        helper=f"collections: {vector.get('collections', {})} / docs: {vector.get('documents_count', 0)}",
    )
    render_metric_card(
        "Neo4j 图数据库",
        "connected True" if graph.get("connected") else "connected False",
        helper=f"nodes: {graph.get('node_count', 0)} / edges: {graph.get('edge_count', 0)}",
    )
    ...
    if latest:
        render_card(
            "最近夜间回顾",
            f"{latest.get('review_date')} / {render_status_badge(latest.get('parse_status', ''))}",
...
def render_long_term_memories() -> None:
    ...
    memory_type = st.selectbox(
        "记忆类型",
        ["全部"] + sorted({str(memory.get("memory_type") or "strategy") for memory in memories}),
    )
...
def render_vector_tab(audit: dict) -> None:
    ...
    cols = st.columns(3)
    cols[0].metric("Chroma available", str(bool(vector.get("available"))))
    cols[1].metric("documents", vector.get("documents_count", vector.get("collection_count", 0)))
    cols[2].metric("embedding", "configured" if embedding.get("configured") else "missing key")
    ...
    render_card(
        ...
        footer=f"collections: {vector.get('collection_names', [])} / counts: {vector.get('collections', {})}",
    )
    if vector.get("error"):
...
def render_graph_tab(audit: dict) -> None:
    ...
    cols = st.columns(3)
    cols[0].metric("Neo4j connected", str(connected))
    cols[1].metric("node count", graph.get("node_count", 0))
    cols[2].metric("edge count", graph.get("edge_count", 0))

    render_card(
        "Neo4j 状态",
        f"backend: {graph.get('backend', 'none')} / uri: {graph.get('uri', '')}",
        footer=(
    ...
    if not connected:
        st.warning(f"{NO_GRAPH_MESSAGE} 失败原因：{graph.get('error') or 'Neo4j 未连接'}")
        return

    nodes = graph.get("sample_nodes") or []
    edges = graph.get("sample_edges") or []
    if not nodes:
        st.info("Neo4j 已连接，但当前暂无节点数据。请运行 scripts/backfill_memory_indexes.py --graph。")
        return
    ...
    for edge in edges:
        ...
        if source in aliases and target in aliases:
            ...
            graph_lines.append(f'{aliases[source]} -> {aliases[target]} [label="{relation}"];')
    graph_lines.append("}")
    st.graphviz_chart("\n".join(graph_lines))

    with st.expander("节点与边列表", expanded=False):
        for node in nodes[:10]:
            st.write(f"{node.get('node_type')} · {node.get('key') or node.get('node_key')} · {node.get('title') or ''}")
...
def render_daily_graph_acceptance_section() -> None:
    ...
    selected = st.selectbox(
        ...
        format_func=lambda item: (
            f"{item.get('graph_date')} / review {item.get('review_id')} / "
            f"nodes {item.get('node_count')} / edges {item.get('edge_count')}"
        ),
    )
    if not selected:
        return
    ...
    cols[2].metric("episodic", len([item for item in memories if item.get("memory_type") == "episodic"]))
    cols[3].metric("semantic", len([item for item in memories if item.get("memory_type") == "semantic"]))

    with st.expander("Daily Graph nodes/edges", expanded=False):
        for node in nodes[:20]:
            st.write(
                f"{node.get('node_type')} / {node.get('node_key')} / "
    ...
    with st.expander("Episodic / Semantic memories for this review", expanded=False):
        for memory in memories:
            render_card(
                f"{memory.get('memory_type')} / memory:{memory.get('id')}",
...
def render_raw_events_tab() -> None:
    ...
    source_filter = st.selectbox(
        "来源过滤",
        ["全部"] + sorted({str(event.get("source_type") or "manual") for event in events}),
    )
    for event in events:
        ...
        render_card(
            ...
            footer=f"{event.get('created_at')} / session {event.get('session_id') or '-'}",
        )
    ...
    for review in reviews:
        ...
        render_json_debug_expander(
            "结构化摘要",
            {
                ...
                "vector_sync_status": (review.get("index_sync_status") or {}).get("vector", {}).get("status", ""),
                "graph_sync_status": (review.get("index_sync_status") or {}).get("graph", {}).get("status", ""),
